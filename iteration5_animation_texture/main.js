...
let container, scene, camera, cameraStartPosition, renderer, controls, loader, raycaster, hoveredItem, clickedItem, sound, sound2, buttons, clickPermission = true;
...
function init() {
    ...
    // Rode cikels die gaan functioneren als knoppen. 
...
function click() {
    if ( clickPermission == true ) {    // De if-statement die voorkomt dat je tijdens de animatie en afspelende audio deze opnieuw kunt starten.
        ...
        if ( intersects.length > 0 ) {
            ...
            if ( clickedItem.id == 6 ) {    // Wanneer geklikt wordt op de telefoon button...
                ...
                createjs.Tween.get( camera.position ).to( { y: 0, x: -25, z: 25 }, 2000, createjs.Ease.getPowInOut( 5 ) ).wait( 6500 ).to( cameraStartPosition, 2000, createjs.Ease.getPowInOut( 5 ) ).call( function() { controls.enabled = true; clickPermission = true; } ) ;
                createjs.Tween.get( controls.target ).to( { x: -35 }, 2000, createjs.Ease.getPowInOut( 5 ) ).wait( 6500 ).to( { x: 0 }, 2000, createjs.Ease.getPowInOut( 5 ) );
                video.play();   // Start de zogenaamde projectie animatie.
            } else if ( clickedItem.id == 7 ) {     // Wanneer geklikt wordt op de vliegtuig button...
                sound2.isPlaying = false;
                setTimeout( function(){ sound2.play(); }, 1000 );
                createjs.Tween.get( camera.position ).to( { y: -5, x: 25, z: 30 }, 2000, createjs.Ease.getPowInOut(5) ).wait( 2100 ).to( cameraStartPosition, 2000, createjs.Ease.getPowInOut( 5 ) ).call( function() { controls.enabled = true; clickPermission = true; } ) ;
